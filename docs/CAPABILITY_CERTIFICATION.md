...
| Company/client isolation | partial | PRs #6, #7, #8, #12, and #15 plus branches `agent/m1-active-scoped-cache-consumer`, `agent/m1-summary-cache-scope-contracts`, and `agent/m1-scope-certification-ticket-health-routing` cover company scope tables, scoped assistant/analytics retrieval, scope snapshots, verifier scope rejection, route matrix, scoped cache/export contracts, active operations-status scoped cache, ticket-health/customer-success summary scoped cache keys, and local ticket-health/customer-success/routing company-scope filters. | Not established. | Production-auth deployment evidence, first-class route exposure decisions, and broader capability Quality Streak evidence remain open. |
