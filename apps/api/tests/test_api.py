...
def test_api_route_authority_matrix_classifies_every_route():
    ...
    company_scoped = {
        ...
        ("GET", "/api/ticket-health/status-duration-sources"),
...
def test_scoped_local_capability_routes_pass_company_scope(monkeypatch):
    ...
    def fake_status_duration_source_candidates_report(authorized_company_ids):
        captured["status_duration_source_candidates_report"] = authorized_company_ids
        return {"ok": True}
    ...
    monkeypatch.setattr(
        "app.main.status_duration_source_candidates_report",
        fake_status_duration_source_candidates_report,
    )
    monkeypatch.setattr(
    ...
    assert client.get("/api/ticket-health/status-duration-sources", headers=headers).status_code == 200
    ...
    assert captured["status_duration_source_candidates_report"] == [123]
